...
const ProjectTimesheet: React.FC<ProjectTimesheetProps> = ({
  ...
  if (tasks.length === 0) {
    return <p className="text-muted-foreground">No tasks assigned to you in this project.</p>;
  }
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b">
                <th className="px-4 py-3 text-left font-medium text-sm text-muted-foreground w-1/4">Task</th>
                <th className="px-4 py-3 text-left font-medium text-sm text-muted-foreground w-1/6">
                  <div className="flex items-center gap-2">
                    Planned Time
                    <Badge variant="outline" className="bg-primary/10 text-primary">Total</Badge>
                  </div>
                </th>
                {dayNames.map((day) => (
                  <th key={day} className="px-4 py-3 text-center font-medium text-sm text-muted-foreground">
                ...
                <th className="px-4 py-3 text-center font-medium text-sm text-muted-foreground">Variance</th>
              </tr>
            </thead>
            <tbody>
              {tasks.map((task) => {
                const taskEntries = timeEntries[task.id] || {};
                const totalMinutes = Object.values(taskEntries).reduce((sum, mins) => sum + mins, 0);
                const estimatedTimeInMinutes = Math.round(task.estimatedTime / 60);
                const variance = totalMinutes - estimatedTimeInMinutes;
                const estimatedTimePerDay = getEstimatedTimePerDay(task.estimatedTime);
                  ...
                  <tr key={task.id} className="border-b hover:bg-muted/50">
                    ...
                    {dayNames.map((day) => (
                      <td key={`${task.id}-${day}`} className="px-4 py-4">
                        <TimeEntry
                          ...
                          onChange={(minutes) => onTimeChange(task.id, day, minutes)}
                    ...
                    <td className="px-4 py-4 text-center">
                      <span className={`font-medium ${
                        ...
                        {variance > 0 ? '+' : ''}{Math.floor(variance / 60)}h {Math.abs(variance % 60)}m
                      </span>
                    </td>
                  </tr>
                );
