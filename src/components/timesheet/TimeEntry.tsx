...
const TimeEntry: React.FC<TimeEntryProps> = ({ 
  ...
  const getTimeComparisonClass = () => {
    ...
    const currentValue = parseInt(hourInput || '0') * 60 + parseInt(minuteInput || '0');
    if (currentValue > estimatedTime) return 'border-orange-400';
    if (currentValue < estimatedTime && currentValue > 0) return 'border-green-400';
    return '';
  };
  
  return (
    <div className="flex items-center w-full justify-center">
      <div className={`flex gap-1 items-center border rounded-md px-2 py-1 ${getTimeComparisonClass()}`}>
        <Input
          type="text"
          value={hourInput}
